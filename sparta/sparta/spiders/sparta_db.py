...
class mysql():
	...
	def __init__(self, type=0,table='', prefix=''):
		"""初始化
		type是数据库类型选择
		...
		if type == 1:
			self.host = sparta_config.host_wp
			self.user = sparta_config.user_wp
			self.passwd = sparta_config.passwd_wp
			self.db = sparta_config.db_wp
			self.charset = sparta_config.charset_wp
		else:
			self.host = sparta_config.host
			self.user = sparta_config.user
			self.passwd = sparta_config.passwd
			self.db = sparta_config.db
			self.charset = sparta_config.charset

		if prefix:
			self.prefix = prefix + '_'
		elif type == 0 and sparta_config.prefix:
			self.prefix = sparta_config.prefix + '_'
		elif type == 1 and sparta_config.prefix_wp:
			self.prefix = sparta_config.prefix_wp + '_'
		else:
			self.prefix = ''
	...
	def add(self, data):
		...
		if hasattr(self, 'tablename'):
			...
			sql = "replace into `" + self.tablename + "` (" + ks[:-1] + ") values (" + vs[:-1] + ")"
			try:
				self.__update(sql)
